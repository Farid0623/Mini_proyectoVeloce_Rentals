...
public class ProductRepositoryJdbcImpl implements Repository<ProductDto> {
    ...
    public List<ProductDto> listar() throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT p.*, c.name as category FROM product as p " +
    ...
    public ProductDto porId(Long id) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement("SELECT p.*, c.name as category FROM product as p " +
    ...
    public void save(ProductDto productDto) throws SQLException {
        ...
        if (productDto.id() != null && productDto.id() > 0) {
            sql = "UPDATE product set type=?, brand=?, model=?, name=?,id_category=?,price=?, available=? where id=?";
        } else {
            sql = "INSERT INTO product (type, brand, model, name,id_category,price, available) VALUES(?,?,?,?,?,?,?)";
        }
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, productDto.type());
            stmt.setString(2, productDto.brand());
            stmt.setString(3, productDto.model());
            stmt.setString(4, productDto.name());
